...
- **즉석 편집(`E`)** — 발표/검토 중 `E`를 누르면 현재 슬라이드를 그 자리에서 수정·삭제(위 라이브 미리보기·아래 편집창), `Cmd/Ctrl+S`로 `slides.md` 저장.
- **다이어그램·차트** — `::: bars`(막대 그래프)·`::: flow`(흐름도/파이프라인)을 마크다운으로. 의존성 0.
- **자가 점검 린터** — `lint.py` 로 출처 누락·내용 넘침·깨진 이미지·제목 누락을 강의 전 자동 점검.
- **PPTX/PDF 임포터** — `import_deck.py` 로 기존 발표자료를 `slides.md` 초안 + 이미지로 변환.
